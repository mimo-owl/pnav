...
Usage:
    python evaluation/scripts/aggregate_runs.py \
        --run gemini:1:eval_results/gemini_run1 \
        --run gemini:2:eval_results/gemini_run2 \
        --run gemini:3:eval_results/gemini_run3 \
        --run gpt:1:eval_results/gpt_run1 \
        --run gpt:2:eval_results/gpt_run2 \
        --run gpt:3:eval_results/gpt_run3 \
        --output-dir analysis/

Each --run argument is: model_name:run_id:path/to/eval_dir
  model_name : arbitrary label (e.g. "gemini", "gpt", "gemma", "claude")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Directory to write CSV files (default: analysis/)",
    )
