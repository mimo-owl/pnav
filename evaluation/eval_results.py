...
Loads predictions from run_vlm_eval.py, computes localization error against
ground-truth 3D positions (projected to 2D), and produces summary plots.

Usage (directory of per-house JSON files — output of merge step):
    python evaluation/scripts/eval_results.py \
        --predictions  eval_results/test0517test0 \
        --dataset-dir  dataset/tmp_0517test0_house500 \
        --output-dir   eval_results/test0517test0

Usage (single flat predictions file — legacy):
    python evaluation/scripts/eval_results.py \
        --predictions  eval_results/test0517test0/predictions.json \
        --dataset-dir  dataset/tmp_0517test0_house500 \
        --output-dir   eval_results/test0517test0

Add --no-plot to skip plot and overlay generation.

Coordinate system note:
    AI2THOR uses Unity's left-handed world space: X=right, Y=up, Z=forward.
    Camera orientation is given by cam_yaw (Y-rotation, degrees) and
    cam_horizon (X-rotation = pitch, degrees; positive = looking down).
    Projected 2D coordinates are normalized to [0, 1]:
...
def _evaluate_prediction(
    ...
    # Normalize predicted pixel coordinates → [0, 1] to match GT
    # Predictions are in pixels: x ∈ [0, sw-1], y ∈ [0, sh-1]
    px = pred["predicted_x"] / sw
    py = pred["predicted_y"] / sh
...
def _aggregate(evals: list[dict]) -> dict:
    ...
    return {
        ...
        "per_type":      {k: _group_stats(v) for k, v in sorted(per_type.items())},
        "per_direction": {k: _group_stats(v) for k, v in sorted(per_dir.items())},
    }
...
def _load_predictions(predictions_path: Path) -> tuple[list[dict], dict]:
    ...
    if predictions_path.is_dir():
        ...
        for json_path in sorted(predictions_path.glob("*.json")):
            if json_path.name in ("meta.json",):
                continue
            try:
...
def evaluate(
    ...
    ov = summary["overall"]
    if ov["n"]:
        print(f"  Overall mean L2   : {ov['mean_l2']:.4f} ± {ov['std_l2']:.4f}")
        print(f"  GT in-frame       : {ov['pct_in_frame']:.1f}%")
    print("\n  Per sentence type:")
    for t, s in summary["per_type"].items():
        if s["n"]:
            print(f"    Type {t.upper()}: n={s['n']}  mean_L2={s['mean_l2']:.4f} ± {s['std_l2']:.4f}"
                  f"  in-frame={s['pct_in_frame']:.1f}%")
    print("\n  Per direction:")
    for d, s in summary["per_direction"].items():
        if s["n"]:
            print(f"    {d:8s}: n={s['n']}  mean_L2={s['mean_l2']:.4f} ± {s['std_l2']:.4f}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--use-verified-surface", action="store_true",
        help="Correct GT height using VERIFY_DIRECTION surface verdicts stored in stage3. "
             "Fixes mismatches between the VLM-judged resting surface and the code-derived "
             "placement height (e.g. VLM says floor but GT was placed at table height). "
             "Original behaviour is preserved when this flag is absent.",
    )
